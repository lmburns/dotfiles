...
function M.setup()
  ...
  local zsh = vim.list_extend(
      default, { aug({ "elif", "if" }), aug({ "((:))", "[[:]]" }, false) }
  )

  local typescript = vim.list_extend(
      default, { aug({ "let", "const", "var" }), aug({ "===", "!==" }) }
  )

  local javascript = vim.list_extend(
      typescript, { aug({ "public", "private", "protected" }) }
  )

  local go = vim.list_extend(
      go, {
        aug({ ":=", "=" }),
...
---@param group table
---@param ft string
local function inc_dec_augroup(group, ft)
...
local function init()
  ...
  local id = create_augroup("lmb__DialIncDec")
  inc_dec_augroup(id, "lua")
  inc_dec_augroup(id, "python")
  inc_dec_augroup(id, "sh")
  inc_dec_augroup(id, "zsh")
  inc_dec_augroup(id, "typescript")
  inc_dec_augroup(id, "javascript")
  inc_dec_augroup(id, "go")
  inc_dec_augroup(id, "vim")
end
