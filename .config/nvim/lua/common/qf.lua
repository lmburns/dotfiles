...
function M.batch_sub(is_loc, pat_rep)
  local matches = fn.matchlist(
      pat_rep, [[\v(([^|"\\a-zA-Z0-9]).*\2.*\2=)([cgeiI]*)\s*$]]
  )
  ...
  local set_cmd = is_loc and function(...) return fn.setloclist(0, ...) end or
                      fn.setqflist
  ...
  local ok, res = pcall(
      function()
        local concat = ([[%s\%%#%s%s%s%s%s]]):format(
            delm, pat, delm, rep, delm, flag
        )
...
function M.qftf(qinfo)
  local items
  local ret = {}
  require("common.gittool").cd_root(fn.bufname("#"), true)
...
local function init()
  ...
  cmd [[
      ...
      com! -nargs=1 Ldos lua require('common.qf').batch_sub(true, <q-args>)
