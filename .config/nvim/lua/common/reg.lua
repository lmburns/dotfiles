...
local RegFloatWin = setmetatable(
    { bufnr = nil, winid = nil, ns = nil, bremap = nil, reg = nil }, {
...
function RegFloatWin:write_buf()
  local lines = vim.tbl_map(function(e) return e.line end, self.rlines)
  local bufnr = self.bufnr
  vim.bo[bufnr].modifiable = true
  api.nvim_buf_set_lines(bufnr, 0, -1, false, lines)
  for i, e in ipairs(self.rlines) do
    api.nvim_buf_set_extmark(
        bufnr, self.ns, i - 1, 1, { hl_group = e.hl_group, end_col = 2 }
    )
  end
...
function M.peek(prefix)
  ...
  if wait or mode == "c" then
    ...
    vim.schedule(function() RegFloatWin:run() end)
    if mode == "i" then
...
local function init()
  ...
  local bremap = setmetatable(
      { n = { ["'"] = "\"", [";"] = "+" }, i = { ["'"] = "\"", [";"] = "+" } },
      { __index = function(t) return t.n end }
  )
  bremap.v = bremap.n
  bremap.c = bremap.i
  ...
  map("n", "\"", [[v:lua.require'common.reg'.peek('"')]], { expr = true })
  map("x", "\"", [[v:lua.require'common.reg'.peek('"')]], { expr = true })
  map("!", "<C-r>", [[v:lua.require'common.reg'.peek('<C-r>')]], { expr = true })
  map("s", "<C-r>", [[v:lua.require'common.reg'.peek('<C-r>')]], { expr = true })
end
