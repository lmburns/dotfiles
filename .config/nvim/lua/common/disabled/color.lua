...
function Color:to_hex(with_alpha)
    local n =
        bit.bor(bit.bor((self.blue * 0xff), bit.lshift((self.green * 0xff), 8)), bit.lshift((self.red * 0xff), 16))
...
function Color:set_from_hsv(x, s, v, a)
    ...
    elseif #x >= 3 then
        ...
        local hsv = self:to_hsv()
        c = Color.from_hsv(x.hue or hsv.hue, x.saturation or hsv.saturation, x.value or hsv.value, self.alpha)
    end
...
function Color:set_from_hsl(x, s, l, a)
    ...
    elseif #x >= 3 then
        ...
        local hsl = self:to_hsl()
        c = Color.from_hsl(x.hue or hsl.hue, x.saturation or hsl.saturation, x.lightness or hsl.lightness, self.alpha)
    end
    return self:set_from_color(c)
end
