...
--NOTE: Maybe delete
---Increase locks, and execute an item in the queue
---@param force? boolean
function Semaphore:signal(force)
    self:release(force)
    if self:has_tasks() then
        if self.count > 1 then
            log.warn("locks shouldn't be available when a task is queued")
