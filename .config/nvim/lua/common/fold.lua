    ...
    if force or 0 < fsize and fsize < 131072 then
      if fn.bufwinid(bufnr) == -1 then
        ...
        api.nvim_buf_call(
            bufnr, function()
              utils.cool_echo(
                  ("bufnr: %d is using anyfold"):format(bufnr), "WarningMsg"
              )
              cmd("AnyFoldActivate")
            end
        )
  ...
  api.nvim_buf_call(
      bufnr, function()
        ...
        for _, f in ipairs(ranges) do
          table.insert(
              fold_cmd, ("%d,%d:fold"):format(f.startLine + 1, f.endLine + 1)
          )
        end
...
function M.update_fold(bufnr)
  ...
  coc.run_command(
      "kvs.fold.foldingRange", { bufnr }, function(e, r)
...
function M.attach(bufnr, force)
  ...
  if vim.g.coc_service_initialized == 1 and
      not vim.tbl_contains(anyfold_prefer_ft, vim.bo.ft) then
    coc.run_command(
        "kvs.fold.foldingRange", { bufnr }, function(e, r)
          ...
          if e == vim.NIL and type(r) == "table" then
            ...
            if winid == -1 then
              cmd(
                  ("au FoldLoad BufEnter <buffer=%d> ++once %s"):format(
                      bufnr, [[lua require('common.fold').update_fold()]]
                  )
              )
            else
              apply_fold(bufnr, r)
