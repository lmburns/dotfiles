...
setlocal indentexpr=MyGetVimIndent()
...
if exists("*MyGetVimIndent")
  finish
endif
let s:keepcpo= &cpo
set cpo&vim

function MyGetVimIndent()
...
function GetVimIndentIntern()
  ...
  if cur_text =~ '^\s*\(ene\@!\|en\%[dif]\|endw\%[hile]\|endfo\%[r]\|endt\%[ry]\|endf\%[unction]\|enddef\|cat\%[ch]\|fina\%[lly]\|el\%[se]\|aug\%[roup]\s*!\=\s\+[eE][nN][dD]\)'
    let ind = ind - shiftwidth()
    if ind < 0
      let ind = 0
    endif
...
"     endif
"
"     " '^\s*\(endif\|endwh\|endfor\|endtry\|endfu\|enddef\|cat\|finall\|else\|aug\%[roup]\s\+[eE][nN][dD]\)'
"     " if getline(v:lnum) =~ '^\s*\(ene\@!\|cat\|fina\|el\|aug\%[roup]\s*!\=\s\+END\)'
"
"     if getline(v:lnum) =~ '^\s*\(ene\@!\|en\%[dif]\|endw\%[hile]\|endfo\%[r]\|endt\%[ry]\|endf\%[unction]\|cat\%[ch]\|fina\%[lly]\|el\%[se]\|aug\%[roup]\s*!\=\s\+[eE][nN][dD]\)'
"         let l:ind = l:ind - &sw
"     endif
"
"     return l:ind
