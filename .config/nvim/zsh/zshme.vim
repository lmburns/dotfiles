...
syn keyword zshConditional      then else fi select esac
...
" TODO: add builtin highlight of ${#var} etc
...
exe 'syn region  zshSubst       matchgroup=zshSubstDelim transparent start=/\$(\ze[^(]/ skip=/\\)/ end=/)/ contains=' . s:contained . '  fold'
exe 'syn region  zshSubstQuoted matchgroup=zshSubstDelim transparent start=/\$(\ze[^(]/ skip=/\\)/ end=/)/ contains=' . s:contained . '  fold'
