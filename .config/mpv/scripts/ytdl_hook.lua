...
local safe_protos = Set {
  "http", "https", "ftp", "ftps", "rtmp", "rtmps", "rtmpe", "rtmpt", "rtmpts",
  "rtmpte", "data"
}
...
local function edl_escape(url) return "%" .. string.len(url) .. "%" .. url end
...
local function parse_yt_playlist(url, json)
  ...
  if maybe_idx and #json.entries >= maybe_idx and json.entries[maybe_idx].id ==
      args["v"] then
...
local function add_single_video(json)
  ...
  elseif reqfmts then
    ...
    for _, track in pairs(reqfmts) do
      ...
      edl_track = edl_track_joined(track.fragments, track.protocol,
                                   json.is_live, track.fragment_base_url)
  ...
  elseif not (json.url == nil) then
    local edl_track = nil
    edl_track = edl_track_joined(json.fragments, json.protocol, json.is_live,
  ...
  if max_bitrate > 0 and not option_was_set("hls-bitrate") and
  ...
  -- set start time
  if not (json.start_time == nil) and not option_was_set("start") and
  ...
  if not (json.stretched_ratio == nil) and
    ...
    mp.set_property('file-local-options/video-aspect-override',
                    json.stretched_ratio)
  end
  ...
  if (json.protocol == "rtmp") then
    ...
    stream_opts = append_libav_opt(stream_opts, "rtmp_swfverify",
                                   json.player_url)
...
function run_ytdl_hook(url)
  ...
  local command = {
    ytdl.path, "--no-warnings", "-J", "--flat-playlist", "--sub-format",
    "ass/srt/best"
  }
  ...
  elseif (json["_type"] == "playlist") or (json["_type"] == "multi_video") then
    ...
    local self_redirecting_url =
        json.entries[1]["_type"] ~= "url_transparent" and
            json.entries[1]["webpage_url"] and json.entries[1]["webpage_url"] ==
            json["webpage_url"]

    -- some funky guessing to detect multi-arc videos
    if self_redirecting_url and #json.entries > 1 and json.entries[1].protocol ==
        "m3u8_native" and json.entries[1].url then
