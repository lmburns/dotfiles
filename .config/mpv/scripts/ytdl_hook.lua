...
local o = {exclude = "", try_ytdl_first = false, use_manifests = false}
options.read_options(o)

local ytdl = {path = "youtube-dl", searched = false, blacklisted = {}}
...
local function parse_yt_playlist(url, json)
    ...
    if maybe_idx and #json.entries >= maybe_idx and json.entries[maybe_idx].id == args["v"] then
...
local function make_absolute_url(base_url, url)
    ...
    local proto, domain, rest = base_url:match("(https?://)([^/]+/)(.*)/?")
    local segs = {}
    rest:gsub(
        "([^/]+)",
        function(c)
    ...
    return proto .. domain .. table.concat(resolved_url, "/")
end
...
local function edl_track_joined(fragments, protocol, is_live, base)
    ...
    if (protocol == "http_dash_segments") and not is_live then
        ...
        for i = offset, #fragments do
            if not fragments[i].duration then
                msg.error("EDL doesn't support fragments" .. "without duration with MP4 DASH")
                return nil
            end
        end
    end
    ...
    for i = offset, #fragments do
        ...
        if fragment.duration then
            parts[#parts] = parts[#parts] .. ",length=" .. fragment.duration
        end
    end
...
local function valid_manifest(json)
    ...
    return (proto == "http_dash_segments" and has_native_dash_demuxer()) or proto:find("^m3u8")
end
...
local function add_single_video(json)
    ...
    if o.use_manifests and valid_manifest(json) then
        -- DASH/split tracks
        local mpd_url = reqfmts and reqfmts[1]["manifest_url"] or json["manifest_url"]
        if not mpd_url then
    ...
    elseif reqfmts then
        ...
        for _, track in pairs(reqfmts) do
            local edl_track = nil
            edl_track = edl_track_joined(track.fragments, track.protocol, json.is_live, track.fragment_base_url)
    ...
    elseif not (json.url == nil) then
        local edl_track = nil
        edl_track = edl_track_joined(json.fragments, json.protocol, json.is_live, json.fragment_base_url)
    ...
    if max_bitrate > 0 and not option_was_set("hls-bitrate") and not option_was_set_locally("hls-bitrate") then
    ...
    if not (json.requested_subtitles == nil) then
        for lang, sub_info in pairs(json.requested_subtitles) do
            ...
            elseif not (sub_info.url == nil) and url_is_safe(sub_info.url) then
                sub = sub_info.url
            end

            if not (sub == nil) then
                mp.commandv("sub-add", sub, "auto", sub_info.ext, lang)
            else
    ...
    -- set start time
    if not (json.start_time == nil) and not option_was_set("start") and not option_was_set_locally("start") then
    ...
    if not (json.stretched_ratio == nil) and not option_was_set("video-aspect-override") then
    ...
    if (json.protocol == "rtmp") then
        stream_opts = append_libav_opt(stream_opts, "rtmp_tcurl", streamurl)
        stream_opts = append_libav_opt(stream_opts, "rtmp_pageurl", json.page_url)
        stream_opts = append_libav_opt(stream_opts, "rtmp_playpath", json.play_path)
        stream_opts = append_libav_opt(stream_opts, "rtmp_swfverify", json.player_url)
        stream_opts = append_libav_opt(stream_opts, "rtmp_swfurl", json.player_url)
        stream_opts = append_libav_opt(stream_opts, "rtmp_app", json.app)
    end

    if json.proxy and json.proxy ~= "" then
        stream_opts = append_libav_opt(stream_opts, "http_proxy", json.proxy)
    end
...
local function check_version(ytdl_path)
    ...
    -- sanity check
    if (tonumber(year) < 2000) or (tonumber(month) > 12) or (tonumber(day) > 31) then
        return
    end
...
function run_ytdl_hook(url)
    ...
    elseif (json["_type"] == "playlist") or (json["_type"] == "multi_video") then
        ...
        local self_redirecting_url =
            json.entries[1]["_type"] ~= "url_transparent" and json.entries[1]["webpage_url"] and
            json.entries[1]["webpage_url"] == json["webpage_url"]

        -- some funky guessing to detect multi-arc videos
        if
            self_redirecting_url and #json.entries > 1 and json.entries[1].protocol == "m3u8_native" and
                json.entries[1].url
         then
            ...
            if not (entry_wsubs == nil) and not (json.entries[entry_wsubs].duration == nil) then
                for j, req in pairs(json.entries[entry_wsubs].requested_subtitles) do
                    local subfile = "edl://"
                    for i, entry in pairs(json.entries) do
                        if
                            not (entry.requested_subtitles == nil) and not (entry.requested_subtitles[j] == nil) and
        ...
        elseif self_redirecting_url and #json.entries == 1 then
            ...
            for i, entry in pairs(json.entries) do
                ...
                if not site:find("://") then
                    ...
                    local prefix = site:find(":") and "ytdl://" or "https://youtu.be/"
            ...
            if use_playlist and not option_was_set("playlist-start") and playlist_index then
...
mp.add_hook(
    o.try_ytdl_first and "on_load" or "on_load_fail",
    10,
    function()
        msg.verbose("full hook")
        local url = mp.get_property("stream-open-filename", "")
        if not (url:find("ytdl://") == 1) and not ((url:find("https?://") == 1) and not is_blacklisted(url)) then
            return
        end
        run_ytdl_hook(url)
    end
