...
if not device then
    if os.getenv("windir") ~= nil then
        device = "windows"
    elseif
        os.execute '[ -d "/Applications" ]' == 0 and os.execute '[ -d "/Library" ]' == 0 or
            os.execute '[ -d "/Applications" ]' == true and os.execute '[ -d "/Library" ]' == true
     then
        device = "mac"
    else
        device = "linux"
...
function paste()
    ...
    elseif
        (filePath == nil) and not has_extension(extensions, currentVideoExtension) and
            not (starts_protocol(protocols, videoFile))
     then
...
function paste_playlist()
    ...
    if
        has_extension(extensions, currentVideoExtension) and (currentVideoExtensionPath ~= "") or
            (starts_protocol(protocols, videoFile))
     then
...
if device == "mac" then -- MAC OS Keybinds
    ...
    mp.add_key_binding("ctrl+p", "paste", paste)
    mp.add_key_binding("ctrl+P", "pasteCaps", paste)

    mp.add_key_binding("ctrl+alt+c", "copy-path", copy_path)
    mp.add_key_binding("ctrl+alt+C", "copy-pathCaps", copy_path)
    mp.add_key_binding("ctrl+alt+p", "paste-playlist", paste_playlist)
    mp.add_key_binding("ctrl+alt+P", "paste-playlistCaps", paste_playlist)
end
