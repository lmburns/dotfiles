...
function init()
    ...
    mp.register_script_message(
        "list-audio-devices",
        function()
            audio:msg_handler("list")
        end
    )
    mp.register_script_message(
        "set-audio-device",
        function(...)
            audio:msg_handler("cycle", ...)
        end
    )
    mp.register_script_message(
        "cycle-audio-devices",
    ...
    mp.add_key_binding(
        ...
        "htp",
        function(e)
            step:key_handler(e, "forward", true)
        end,
        {complex = true}
    )
    mp.add_key_binding(
        ...
        "htp-back",
        function(e)
            step:key_handler(e, "backward", true)
        end,
        {complex = true}
    )
end
    ...
    playback = {
        ...
        rewind = function(playlist_pos, pause)
            if playlist_pos then
                mp.set_property("playlist-pos-1", math.min(playlist_pos, get("playlist-count")))
            end
            mp.add_timeout(
                0.01,
                function()
    ...
    get_fps = function(self)
        ...
        local fps =
            ...
            (self.drop_delta > 0 and
                (pos_delta * mult > 2 or
                    pos_delta * mult / self.interval > speed * 0.95 and self.frames * mult > 18 * self.interval) and
...
osd = {
    ...
    osd_timer = mp.add_timeout(
        1e8,
        function()
            mp.set_property("osd-msg1", "")
        end
    ),
    msg_timer = mp.add_timeout(
        1e8,
        function()
            osd.msg = osd.default_msg()
        end
    ),
...
fullscreen = {
    ...
    on_click = function(self)
        ...
        if
            self.clicks == 1 and mp.get_time() - self.prev_time < self.time_window and
                math.abs(mp.get_mouse_pos() - self.x) < 5
         then
            self.clicks = 2
        else
...
step = {
    ...
    delay_timer = mp.add_timeout(
        1e8,
        function()
            step:play()
        end
    ),
    hwdec_timer = mp.add_periodic_timer(
        1 / 60,
        function()
            if
                get("play-dir") == "forward" and not media.playback.paused and
                    get("estimated-frame-number") ~= step.dir_frame
             then
    ...
    on_pause = function(self, pause)
        if not pause and self.stepped then
            if
                (o.step_mute == "auto" and not self.muted) or
                    (o.step_mute == "hold" and not self.muted and not self.played)
             then
    ...
    start = function(self, dir, htp)
        ...
        if htp then
            ...
            if dir == "forward" and o.step_method == "step" then
                ...
                self.stepped = true
            elseif
                (dir == "backward" and get("time-pos") > 0) or (dir == "forward" and get("time-pos") < get("duration"))
             then
    ...
    stop = function(self, dir, htp)
        ...
        if
            dir == "backward" and get("estimated-frame-number") > 0 and not self.played and
                media.playback.time == self.prev_pos
         then
