...
function init()
  ...
  if o.audio_device > 0 then
    audio:set(o.audio_device, get('volume'))
  end
  if o.step_delay == -1 then
    o.step_delay = get('input-ar-delay')
  end
  if o.step_rate == -1 then
    o.step_rate = get('input-ar-rate')
  end
  if o.end_rewind == 'file' then
  ...
  osd.default_msg = function()
    ...
    elseif media.type == 'video' then
      return o.video_symbol .. media.playback.frame .. ' / ' .. media.frames ..
                 ' (' .. media.playback.progress .. '%)\n' ..
                 format(media.playback.time) .. '\n' .. round(fps.fps, 3) ..
                 ' fps (' .. round(media.playback.speed, 2) .. 'x)'
    else
      return ''
    end
  end
  ...
  mp.observe_property('window-minimized', 'bool',
                      function(_, v) media.playback:on_minimize(v) end)
  mp.observe_property('estimated-vf-fps', 'number',
                      function(_, v) fps:on_est_fps(v) end)
  ...
  mp.register_script_message('list-audio-devices',
                             function() audio:msg_handler('list') end)
  mp.register_script_message('set-audio-device',
                             function(...) audio:msg_handler('cycle', ...) end)
  mp.register_script_message('cycle-audio-devices',
                             function(...) audio:msg_handler('cycle', ...) end)
  ...
  mp.add_key_binding(nil, 'cycle-fullscreen',
                     function(e) fullscreen:key_handler(e) end,
                     { complex = true })
  mp.add_key_binding(nil, 'step',
                     function(e) step:key_handler(e, 'forward') end,
                     { complex = true })
  mp.add_key_binding(nil, 'step-back',
                     function(e) step:key_handler(e, 'backward') end,
                     { complex = true })
  mp.add_key_binding(nil, 'htp',
                     function(e) step:key_handler(e, 'forward', true) end,
                     { complex = true })
  mp.add_key_binding(nil, 'htp-back',
                     function(e) step:key_handler(e, 'backward', true) end,
                     { complex = true })
end
...
function get(property) return mp.get_property_native(property) end
...
media = {
  ...
  get_type = function(self)
    ...
    for i = 0, tracks - 1 do
      ...
      if get('track-list/' .. i .. '/albumart') then
        self.type = 'audio'
      elseif not get('container-fps') then
        self.type = 'video'
      elseif self.frames == 1 or (not self.duration or self.duration == 0) then
        self.type = 'image'
      else
        self.type = 'video'
      end
  ...
  playback = {
    ...
    play = function(dir, speed)
      ...
      if not step.played_backward or
          (step.played_backward and
              (not step.prev_hwdec or step.prev_hwdec == 'no')) then
    ...
    rewind = function(playlist_pos, pause)
      if playlist_pos then
        mp.set_property('playlist-pos-1',
                        math.min(playlist_pos, get('playlist-count')))
      end
    ...
    on_load = function()
      if o.play_on_load ~= 'auto' then
        mp.command('set pause ' .. (o.play_on_load == 'yes' and 'no' or 'yes'))
      end
    end,
    on_tick = function(self, time)
      ...
      if media.type == 'video' then
        if media.frames > 0 then
          self.frame = math.min(
                           round(media.frames * self.time / media.duration) + 1,
                           media.frames)
          self.eof = media.frames - self.frame <= 1
          self.progress = math.floor((self.frame - (self.eof and 0 or 1)) /
                                         media.frames * 100)
        else
          self.frame = 0
          self.progress = self.eof and 100 or
                              math.floor(self.time / (media.duration or 0) * 100)
        end
      end
    end,
    on_minimize = function(self, minimized)
      ...
      elseif not get('d3d11-flip') or not get('angle-flip') then
        if minimized then
          ...
          if self.prev_sync then
            mp.command('no-osd set video-sync ' .. self.prev_sync)
          end
          self.prev_sync = nil
        end
      end
    ...
    on_eof = function(self, eof)
      self.eof = eof
      if o.end_rewind ~= 'no' and eof and not step.played then
        self.rewind(tonumber(o.end_rewind), true)
      end
    end
  }
}
...
audio = {
  ...
  create_user_list = function(self)
    ...
    if #user_list == 0 then
      user_list = split(o.audio_devices, "'([^']+)'")
    end
    local list = self:get()
  ...
  list = function(self, list, duration)
    ...
    for i, v in ipairs(list) do
      ...
      if v.name == get('audio-device') then
        symbol = (get('mute') or get('volume') == 0) and o.audio_muted_symbol or
                     o.audio_symbol
        if o.show_volume then
          vol = '(' .. get('volume') .. ') '
        end
      end
  ...
  cycle = function(self, list)
    ...
    self.i = u.to_string(list) ~= self.prev_list and 1 or
                 (self.i == #list and 1 or self.i + 1)
    ...
    for i, v in ipairs(list) do
      ...
      if i == self.i then
        ...
        vol = tonumber(components[2]) or self.user_list[index].volume or
                  get('volume')
      end
...
fps = {
  ...
  get_fps = function(self)
    ...
    local function hot_mess(p_speed)
      return
          self.drop_delta > 0 and self.frames * mult < self.est_fps * p_speed /
              math.max(self.est_fps / 30, 1) * self.interval * 0.95 and
              round(self.frames * mult, 2) or self.est_fps * speed
    end
    local fps = speed > 1 and (self.drop_delta > 0 and
                    (pos_delta * mult > 2 or pos_delta * mult / self.interval >
                        speed * 0.95 and self.frames * mult > 18 * self.interval) and
                    round(self.est_fps * pos_delta * mult / self.interval, 2) or
                    hot_mess(1)) or hot_mess(speed)
    if fps > 1 then
      self.fps = fps
    end
    self.prev_time = time
  ...
  on_speed = function(self) self.fps = self.est_fps * media.playback.speed end
}
...
osd = {
  ...
  set = function(self, msg, duration)
    if msg or not self.toggled or
        (self.toggled and self.osd_timer.timeout ~= 1e8) then
      self.osd_timer:kill()
...
fullscreen = {
  ...
  on_click = function(self)
    if mp.get_time() - self.prev_time > self.time_window then
      self.clicks = 0
    end
    if self.clicks == 1 and mp.get_time() - self.prev_time < self.time_window and
        math.abs(mp.get_mouse_pos() - self.x) < 5 then
      self.clicks = 2
    else
  ...
  on_eof = function(eof)
    if o.end_exit_fs and eof and not step.played then
...
step = {
  ...
  hwdec_timer = mp.add_periodic_timer(1 / 60, function()
    if get('play-dir') == 'forward' and not media.playback.paused and
        get('estimated-frame-number') ~= step.dir_frame then
  ...
  on_pause = function(self, pause)
    if not pause and self.stepped then
      if (o.step_mute == 'auto' and not self.muted) or
          (o.step_mute == 'hold' and not self.muted and not self.played) then
  ...
  play = function(self)
    self.played = true
    if o.step_mute == 'auto' and not self.muted then
      mp.command('no-osd set mute no')
    elseif o.step_mute == 'hold' then
      mp.command('no-osd set mute yes')
    end
    if self.direction == 'backward' then
      mp.command('no-osd set hwdec no')
    end
  ...
  start = function(self, dir, htp)
    ...
    if htp then
      ...
      if dir == 'forward' and o.step_method == 'step' then
        if o.step_mute ~= 'no' then
          mp.command('no-osd set mute yes')
        end
        mp.command('frame-step')
        self.stepped = true
      elseif (dir == 'backward' and get('time-pos') > 0) or
          (dir == 'forward' and get('time-pos') < get('duration')) then
        mp.commandv('seek', (dir == 'forward' and 1 or -1) / fps.est_fps,
                    'relative+exact')
      end
    end
  end,
  stop = function(self, dir, htp)
    ...
    if dir == 'backward' and get('estimated-frame-number') > 0 and
        not self.played and media.playback.time == self.prev_pos then
      mp.command('frame-back-step')
      print('Backward seek failed. Reverted to backstep.')
    end
    if not htp or not o.htp_keep_dir then
      mp.command('no-osd set play-dir forward')
    end
    media.playback.speed = self.prev_speed
    mp.command('no-osd set speed ' .. media.playback.speed)
    if o.step_mute ~= 'no' and
        (not self.muted and not (o.step_mute ~= 'no' and self.stepped)) then
      mp.command('no-osd set mute no')
    end
    if (htp and self.paused) or
        (not htp and
            ((o.step_method == 'step' and not self.played) or self.played)) then
      mp.command('set pause yes')
    end
    if self.played and not media.playback.eof then
      mp.commandv('seek', 0, 'relative+exact')
    end
    self.played = false
    if not osd.toggled then
      osd.show = false
    end
  end,
  on_press = function(self, dir, htp)
    local msg = 'Received a key press event.\n' ..
                    (htp and 'Key down/up events are required.\n' or
                        'Only single frame steps will work.\n') ..
