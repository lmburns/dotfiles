...
(function () {
  ...
  var autoQueue = function (filename) {
    ...
    for (i = 0, len = index.files.length; i < len; ++i) {
      ...
      if (wantedExts.indexOf(ext) < 0) continue;
      if (file === currentFile) foundAt = files.length;
      files.push(PathTools.getSubPath(path, file));
    }
    if (foundAt === -1 || files.length <= 1) return; // We didn't find target file, or ONLY found that file.
    ...
    if (isPlaying && mp.get_property_number("playlist-count") === 1) {
      ...
      switch (info) {
        case "dir":
          ...
          resolvingDir = true;
          if (debug) Utils.dump("resolving dir:" + filename);
          break;
        case "file":
          resolvingDir = false;
          ...
          if (wantedExts.indexOf(ext) >= 0) {
            if (debug) Utils.dump("autoload:" + filename);
            autoQueue(filename);
          }
          break;
        default:
