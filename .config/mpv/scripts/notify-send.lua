...
local cover_filenames = {
  ...
  "folder.jpeg", "AlbumArtwork.png", "AlbumArtwork.jpg", "AlbumArtwork.jpeg"
}
...
function notify(summary, body, options)
  ...
  return mp.command_native({
    "run", "notify-send", table.unpack(option_args), summary, body
  })
end
...
function notify_current_media()
  ...
  if metadata then
    function tag(name) return metadata[string.upper(name)] or metadata[name] end
