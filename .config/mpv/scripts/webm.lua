...
expand_properties = function(text, magic)
    ...
    for prefix, raw, prop, colon, fallback, closing in text:gmatch(
        "%" .. magic .. "{([?!]?)(=?)([^}:]*)(:?)([^}]*)(}*)}"
    ) do
        local err
        local prop_value
        ...
        if colon == ":" then
                ...
                "%" ..
                    magic ..
                        "{" ..
                            prefix ..
                                raw ..
                                    original_prop:gsub("%W", "%%%1") ..
                                        ":" .. fallback:gsub("%W", "%%%1") .. closing .. "}",
            ...
            text, _ =
                ...
                "%" ..
                    magic ..
                        "{" .. prefix .. raw .. original_prop:gsub("%W", "%%%1") .. closing .. "}",
                prop_value
            )
        end
    end
...
format_filename = function(startTime, endTime, videoFormat)
    ...
    local replaceTable = {
        ...
        ["%%M"] = (mp.get_property_native("aid") and not mp.get_property_native("mute") and
            hasAudioCodec) and
            "-audio" or
            "",
        ["%%R"] = (options.scale_height ~= -1) and "-" .. tostring(options.scale_height) .. "p" or
            "-" .. tostring(mp.get_property_native("height")) .. "p",
        ["%%t%%"] = "%%"
    }
    ...
    if mp.get_property_bool("demuxer-via-network", false) then
        ...
        local x =
            ...
            string.gsub(mp.get_property("filename", ""), "%W", "%%%1") .. "$",
            ""
        )
        local _
...
local is_windows =
    type(package) == "table" and type(package.config) == "string" and
...
run_subprocess = function(params)
    ...
    if res.status ~= 0 then
        msg.verbose(
            ...
            res.error,
            " Killed by us? ",
            res.killed_by_us and "yes" or "no"
        )
        return false
    end
    return true
...
get_video_dimensions = function()
    ...
    if keep_aspect then
        ...
        split_scaling = function(dst_size, scaled_src_size, zoom, align, pan)
            ...
            align = (align + 1) / 2
            local dst_start =
                math.floor((dst_size - scaled_src_size) * align + pan * scaled_src_size)
            if dst_start < 0 then
...
do
    ...
    self.from_bytes = function(self, bytes)
        ...
        do
            ...
            for _index_0 = self:data_before_duration_size() + 1, _max_0 < 0 and #bytes + _max_0 or
                _max_0 do
                local b = bytes[_index_0]
                _accum_0[_len_0] = b
                _len_0 = _len_0 + 1
    ...
    local _base_0 = {
        getColorspace = function(self)
            ...
            elseif "smpte-240m" == _exp_0 then
                ...
                msg.info(
                    "Warning, unknown colorspace " .. tostring(csp) .. " detected, using bt.601."
                )
                return "bt601"
            end
        end,
...
do
    ...
    local _base_0 = {
        postCommandModifier = function(self, command, region, startTime, endTime)
            ...
            local cfilter =
                "[vid1]trim=start=" ..
                tostring(start_ts) .. ":end=" .. tostring(end_ts) .. "[vidtmp];"
            ...
            for _, v in ipairs(command) do
                ...
                repeat
                    if
                        v:match("^%-%-vf%-add=lavfi%-scale") or v:match("^%-%-vf%-add=lavfi%-crop") or
                            v:match("^%-%-vf%-add=fps") or
                            v:match("^%-%-vf%-add=lavfi%-eq")
                     then
                        ...
                        if v:match("^%-%-video%-rotate=90") then
                            ...
                            if v:match("^%-%-video%-rotate=270") then
                                ...
                                if v:match("^%-%-video%-rotate=180") then
                                    cfilter =
                                        cfilter ..
                                        "[vidtmp]transpose=1[vidtmp];[vidtmp]transpose=1[vidtmp];"
                                else
...
append_track = function(out, track)
    ...
    if track["external"] and string.len(track["external-filename"]) <= 2048 then
        return append(
            ...
            {
                "--" ..
                    tostring(external_flag[track["type"]]) ..
                        "=" .. tostring(track["external-filename"])
            }
        )
    else
        return append(
...
get_contrast_brightness_and_saturation_filters = function()
    ...
    return {
        "lavfi-eq=contrast=" ..
            tostring(eq_contrast) ..
                ":saturation=" ..
                    tostring(eq_saturation) .. ":brightness=" .. tostring(eq_brightness)
    }
end
    ...
    for _index_0 = 1, #vf do
        ...
        repeat
            ...
            for k, v in pairs(params) do
                str =
                    str ..
                    ":" .. tostring(k) .. "=%" .. tostring(string.len(v)) .. "%" .. tostring(v)
            end
            append(
                filters,
                {
            ...
            filters,
            {
                "lavfi-crop=" ..
                    tostring(region.w) ..
                        ":" ..
                            tostring(region.h) ..
                                ":" .. tostring(region.x) .. ":" .. tostring(region.y)
            }
        )
    end
    ...
    if format.acceptsBitrate then
        if options.target_filesize > 0 then
            ...
            local video_bitrate, audio_bitrate =
                calculate_bitrate(supported_active_tracks, format, length)
    ...
    local _base_0 = {
        ...
        draw = function(self)
            ...
            ass:append(
                tostring(bold("1:")) ..
                    " change point A (" ..
                        tostring(self.pointA.x) .. ", " .. tostring(self.pointA.y) .. ")\\N"
            )
            ass:append(
                tostring(bold("2:")) ..
                    " change point B (" ..
                        tostring(self.pointB.x) .. ", " .. tostring(self.pointB.y) .. ")\\N"
            )
            ass:append(tostring(bold("r:")) .. " reset to whole screen\\N")
            ass:append(tostring(bold("ESC:")) .. " cancel crop\\N")
            local width, height =
                math.abs(self.pointA.x - self.pointB.x),
                math.abs(self.pointA.y - self.pointB.y)
            ass:append(
                tostring(bold("ENTER:")) ..
                    " confirm crop (" .. tostring(width) .. "x" .. tostring(height) .. ")\\N"
            )
                ...
                if not set then
                    return msg.warn(
                        "Tried to set invalid value " ..
                            tostring(value) .. " to " .. tostring(self.displayText) .. " option."
                    )
                end
            end
        end,
            ...
            __init = function(self, callback)
                ...
                self.options = {
                    ...
                    },
                    {
                        ...
                        Option(
                            "bool",
                            "Strict Filesize Constraint",
                            options.strict_filesize_constraint
                        )
                    },
                    {
                        ...
                        Option(
                            "bool",
                            "Write Filename on Metadata",
                            options.write_filename_on_metadata
                        )
                    },
                    {
                        "target_filesize",
...
do
    ...
    local _base_0 = {
        ...
        draw = function(self)
            ...
            ass:append(
                tostring(bold("1:")) ..
                    " set start time (current is " ..
                        tostring(seconds_to_time_string(self.startTime)) .. ")\\N"
            )
            ass:append(
                tostring(bold("2:")) ..
                    " set end time (current is " ..
                        tostring(seconds_to_time_string(self.endTime)) .. ")\\N"
            )
